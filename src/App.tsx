...
export default function App() {
  ...
  const [savedMonthlyPlans, setSavedMonthlyPlans] = useState<MonthlyPlan[]>([]);
  const [isLoading, setIsLoading] = useState(false);
  const [loadingType, setLoadingType] = useState<'daily' | 'weekly' | 'monthly' | 'image' | null>(null);
  const [viewMode, setViewMode] = useState<'daily' | 'weekly' | 'monthly' | 'history' | 'saved'>('daily');
  const [error, setError] = useState<string | null>(null);
  const [successMessage, setSuccessMessage] = useState<string | null>(null);
  const [selectedDay, setSelectedDay] = useState<{ weekIdx?: number, dayIdx: number, plan: any } | null>(null);
  const [showPrintPreview, setShowPrintPreview] = useState(false);
  const [isFullscreen, setIsFullscreen] = useState(false);
  const [fontSize, setFontSize] = useState(100);
  const [singleMeal, setSingleMeal] = useState<{ meal: Meal, type: string } | null>(null);
  ...
  useEffect(() => {
    ...
    const savedWeekly = localStorage.getItem('nutrimed_weekly_active');
    const savedMonthly = localStorage.getItem('nutrimed_monthly_active');
    const archivedWeekly = localStorage.getItem('nutrimed_weekly_archive');
    const archivedMonthly = localStorage.getItem('nutrimed_monthly_archive');
    ...
    if (savedProfile) {
      ...
      if (!parsed.likedMeals) parsed.likedMeals = [];
      if (!parsed.dislikedMeals) parsed.dislikedMeals = [];
      if (!parsed.favoriteCuisines) parsed.favoriteCuisines = [];
      if (!parsed.spiceLevel) parsed.spiceLevel = 'medium';
      setProfile(parsed);
    ...
    if (archivedWeekly) setSavedWeeklyPlans(JSON.parse(archivedWeekly));
    if (archivedMonthly) setSavedMonthlyPlans(JSON.parse(archivedMonthly));
  }, []);
  ...
  const handleSaveToArchive = (plan: WeeklyPlan | MonthlyPlan) => {
    if ('weeks' in plan) {
      const newArchive = [plan as MonthlyPlan, ...savedMonthlyPlans].slice(0, 5);
      setSavedMonthlyPlans(newArchive);
      localStorage.setItem('nutrimed_monthly_archive', JSON.stringify(newArchive));
    } else {
      const newArchive = [plan as WeeklyPlan, ...savedWeeklyPlans].slice(0, 10);
      setSavedWeeklyPlans(newArchive);
      localStorage.setItem('nutrimed_weekly_archive', JSON.stringify(newArchive));
    }
    setSuccessMessage(t.planSaved);
  };

  const handleDeleteSavedPlan = (idx: number, type: 'weekly' | 'monthly') => {
    if (type === 'weekly') {
      const newArchive = savedWeeklyPlans.filter((_, i) => i !== idx);
      setSavedWeeklyPlans(newArchive);
      localStorage.setItem('nutrimed_weekly_archive', JSON.stringify(newArchive));
    } else {
      const newArchive = savedMonthlyPlans.filter((_, i) => i !== idx);
      setSavedMonthlyPlans(newArchive);
      localStorage.setItem('nutrimed_monthly_archive', JSON.stringify(newArchive));
    }
  };
  ...
  const handleSaveProfile = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const newProfile: UserProfile = {
      ...
      favoriteCuisines: (formData.get('favoriteCuisines') as string).split(',').map(s => s.trim()).filter(Boolean),
  ...
  const handleExcludeIngredient = async (ingredient: string, mealType?: 'breakfast' | 'lunch' | 'snack' | 'dinner', dayIndex?: number, weekIndex?: number) => {
    if (!profile) return;
    
    // Clean ingredient name (remove amounts)
    const cleanIngredient = ingredient.replace(/^\d+\s*(g|kg|ml|l|oz|lb|tazas|cucharadas|cucharaditas|unidades|piezas|dientes|rebanadas|hojas|pizcas|ramas|tallos|filetes|lonchas|trozos|cubos|bolsas|latas|paquetes|frascos|botellas|vasos|copas|tazones|cuencos|platos|raciones|porciones|servicios)\s*(de\s+)?/i, '').trim();

    const currentForbidden = profile.forbiddenIngredients 
      ? profile.forbiddenIngredients.split(',').map(s => s.trim().toLowerCase())
      : [];

    if (currentForbidden.includes(cleanIngredient.toLowerCase())) {
      setSuccessMessage(lang === 'es' ? `"${cleanIngredient}" ya está en tu lista de prohibidos.` : `"${cleanIngredient}" is already in your forbidden list.`);
    } else {
      // Check if it's a fish
      const fishKeywords = ['salmón', 'salmon', 'lubina', 'atún', 'atun', 'bacalao', 'merluza', 'dorada', 'trucha', 'sardina', 'boquerón', 'boqueron', 'pescado', 'fish', 'tuna', 'cod', 'hake', 'sea bass', 'trout', 'sardine', 'anchovy', 'lenguado', 'rape', 'rodaballo', 'caballa', 'bonito', 'emperador', 'pez espada'];
      const isFish = fishKeywords.some(k => cleanIngredient.toLowerCase().includes(k));

      const newForbidden = profile.forbiddenIngredients 
        ? `${profile.forbiddenIngredients}, ${cleanIngredient}`
        : cleanIngredient;
      ...
      if (isFish && lang === 'es') {
        setSuccessMessage(`Pescado detectado: "${cleanIngredient}" añadido a la lista de no deseados.`);
      } else {
        setSuccessMessage(lang === 'es' ? `"${cleanIngredient}" añadido a la lista de prohibidos.` : `"${cleanIngredient}" added to forbidden list.`);
      }
    }
    
    // Regenerate ONLY that specific meal if context is provided
    if (viewMode === 'daily' && mealType) {
      await handleRegenerateMeal(mealType);
    } else if (viewMode === 'weekly' && mealType && dayIndex !== undefined) {
      await handleRegenerateWeeklyMeal(dayIndex, mealType);
    } else if (viewMode === 'monthly' && mealType && dayIndex !== undefined && weekIndex !== undefined) {
      await handleRegenerateMonthlyMeal(weekIndex, dayIndex, mealType);
    } else {
      // Fallback to full regeneration if no specific meal context
  ...
  const handleRegenerateWeeklyMeal = async (dayIndex: number, mealType: 'breakfast' | 'lunch' | 'snack' | 'dinner') => {
    ...
    try {
      ...
      const newMeal = await regenerateSingleMeal(profile, dayPlan, mealType, lang);
      ...
      // Recalculate nutrition for that day
      const meals = [updatedDays[dayIndex].breakfast, updatedDays[dayIndex].lunch, updatedDays[dayIndex].snack, updatedDays[dayIndex].dinner];
      updatedDays[dayIndex].totalNutrition = {
        ...updatedDays[dayIndex].totalNutrition,
      ...
      localStorage.setItem('nutrimed_weekly_active', JSON.stringify(updatedWeeklyPlan));
    } catch (err) {
  ...
  const handleRegenerateMonthlyMeal = async (weekIndex: number, dayIndex: number, mealType: 'breakfast' | 'lunch' | 'snack' | 'dinner') => {
    if (!profile || !monthlyPlan) return;
    setIsLoading(true);
    setLoadingType('monthly');
    setError(null);
    try {
      const { regenerateSingleMeal } = await import('./services/gemini');
      const dayPlan = monthlyPlan.weeks[weekIndex].days[dayIndex];
      const newMeal = await regenerateSingleMeal(profile, dayPlan, mealType, lang);
      
      const updatedWeeks = [...monthlyPlan.weeks];
      ...
      const updatedMonthlyPlan = {
        ...monthlyPlan,
        weeks: updatedWeeks
      };

      setMonthlyPlan(updatedMonthlyPlan);
      localStorage.setItem('nutrimed_monthly_active', JSON.stringify(updatedMonthlyPlan));
    } catch (err) {
      setError(lang === 'es' ? 'Error al regenerar la receta.' : 'Error regenerating meal.');
      console.error(err);
    } finally {
  ...
  const handleGenerateSingleMeal = async (mealType: 'breakfast' | 'lunch' | 'snack' | 'dinner') => {
    if (!profile) return;
    setIsLoading(true);
    setLoadingType('daily');
    setError(null);
    try {
      const { generateSingleMeal } = await import('./services/gemini');
      const meal = await generateSingleMeal(profile, mealType, lang);
      
      // Automatically generate image for single meal
      try {
        const { generateMealImage } = await import('./services/gemini');
        const imageUrl = await generateMealImage(meal.name, meal.description);
        meal.imageUrl = imageUrl;
      } catch (imgErr) {
        console.error("Error generating automatic image:", imgErr);
      }

      setSingleMeal({ meal, type: mealType });
      // Clear other plans to focus on the single meal
      setCurrentPlan(null);
      setWeeklyPlan(null);
      setMonthlyPlan(null);
      setViewMode('daily');
    } catch (err) {
      setError(lang === 'es' ? 'Error al generar la receta.' : 'Error generating recipe.');
      console.error(err);
    } finally {
      setIsLoading(false);
      setLoadingType(null);
  ...
  const handleLikeMeal = (mealName: string) => {
    if (!profile) return;
    const likedMeals = [...(profile.likedMeals || [])];
    if (!likedMeals.includes(mealName)) {
      likedMeals.push(mealName);
      const newProfile = { ...profile, likedMeals };
      setProfile(newProfile);
      localStorage.setItem('nutrimed_profile', JSON.stringify(newProfile));
      setSuccessMessage(lang === 'es' ? '¡Me alegra que te guste! Aprenderé de esto.' : "Glad you like it! I'll learn from this.");
    }
  };

  const handleDislikeMeal = (mealName: string) => {
    if (!profile) return;
    const dislikedMeals = [...(profile.dislikedMeals || [])];
    if (!dislikedMeals.includes(mealName)) {
      dislikedMeals.push(mealName);
      const newProfile = { ...profile, dislikedMeals };
      setProfile(newProfile);
      localStorage.setItem('nutrimed_profile', JSON.stringify(newProfile));
      setSuccessMessage(lang === 'es' ? 'Entendido, evitaré platos similares.' : "Understood, I'll avoid similar dishes.");
    }
  };
  ...
  const handleRegenerateMeal = async (mealType: 'breakfast' | 'lunch' | 'snack' | 'dinner') => {
    if (!profile || !currentPlan) return;
    setIsLoading(true);
    setError(null);
    try {
      const { regenerateSingleMeal } = await import('./services/gemini');
      const newMeal = await regenerateSingleMeal(profile, currentPlan, mealType, lang);
      ...
      updatedPlan.totalNutrition = {
        ...
        fiber: Math.round(meals.reduce((sum, m) => sum + m.nutritionalInfo.fiber, 0)),
        saturatedFat: Math.round(meals.reduce((sum, m) => sum + m.nutritionalInfo.saturatedFat, 0) * 10) / 10,
        glycemicLoad: currentPlan.totalNutrition.glycemicLoad
      };
  ...
  const handleGenerateWeekly = async () => {
    ...
    try {
      ...
      setWeeklyPlan(plan);
      localStorage.setItem('nutrimed_weekly_active', JSON.stringify(plan));
      setCurrentPlan(null);
      setMonthlyPlan(null);
    } catch (err) {
      console.error(err);
  ...
  const handleGenerateMonthly = async () => {
    ...
    try {
      ...
      setMonthlyPlan(plan);
      localStorage.setItem('nutrimed_monthly_active', JSON.stringify(plan));
      setWeeklyPlan(null);
      setCurrentPlan(null);
    } catch (err) {
      console.error(err);
  ...
  const handleGenerateAllImages = async () => {
    ...
    try {
      ...
      // Limit to 7 days if it's a monthly plan to avoid overwhelming
      const daysToProcess = 'weeks' in plan ? allDays.slice(0, 7) : allDays;
  ...
  const handleDownloadPDF = async () => {
    ...
    if (viewMode === 'monthly') {
      setError(lang === 'es' ? 'La descarga de PDF para el mes completo es muy pesada. Por favor, descarga semana a semana.' : 'Monthly PDF download is too large. Please download week by week.');
      return;
    }

    setIsLoading(true);
    setLoadingType('image'); // Reusing image loading for PDF generation
    try {
      ...
      const planData = viewMode === 'weekly' ? weeklyPlan : currentPlan;
  ...
  const generateFullHTML = (planData: any, viewMode: string) => {
      ...
      <div class="bg-white p-4 rounded-xl border border-stone-200 shadow-sm space-y-2">
        ...
        <div class="flex justify-between items-start">
          ...
          <div class="flex gap-2">
            ${meal.prepTime ? `<span class="text-[10px] font-bold text-med-olive bg-med-olive/5 px-2 py-0.5 rounded" title="${t.prepTime}">⏱ ${meal.prepTime}</span>` : ''}
            ${meal.cookingTime ? `<span class="text-[10px] font-bold text-orange-600 bg-orange-50 px-2 py-0.5 rounded" title="${t.cookingTime}">🔥 ${meal.cookingTime}</span>` : ''}
          </div>
        </div>
        ...
        ${meal.proTip ? `
          <div class="bg-stone-50 border-l-4 border-med-olive p-3 rounded-r-lg my-3">
            <span class="block text-[10px] font-bold uppercase text-med-olive mb-1">${t.proTip}</span>
            <p class="text-xs italic text-stone-700">${meal.proTip}</p>
          </div>
        ` : ''}
    ...
    const generateDayHTML = (day: DailyMealPlan) => `
      <div class="space-y-4 p-6 bg-stone-50/50 rounded-2xl border border-stone-200">
        <div class="flex justify-between items-center border-b border-stone-200 pb-2">
          <h2 class="text-2xl font-serif text-stone-800">${day.date}</h2>
          <div class="flex gap-2 text-[10px] font-bold uppercase text-stone-400">
            ${profile?.favoriteCuisines?.length ? `<span>Cuisine: ${profile.favoriteCuisines.join(', ')}</span>` : ''}
            ${profile?.spiceLevel ? `<span>Spice: ${profile.spiceLevel}</span>` : ''}
          </div>
        </div>
        ...
        <div class="text-center space-y-8 py-12">
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mt-12">
             ...
             <div class="p-8 bg-white rounded-3xl shadow-sm border border-stone-200">
                ...
                <p class="text-sm text-stone-500">Instrucciones passo a passo generadas específicamente para ti.</p>
             </div>
          </div>
        </div>
      `;
...
<body class="p-4 md:p-8">
  ...
  const handleDownloadHTML = () => {
    ...
    const htmlContent = generateFullHTML(planData, viewMode);
  ...
  const handleShareWhatsApp = async () => {
    ...
    // 1. Generate Text Summary (Detailed)
    let textSummary = `*NutriMed AI - ${t.subtitle}*\n\n`;
    
    if (viewMode === 'daily' && currentPlan) {
      textSummary += `📅 *${currentPlan.date}*\n`;
      const meals = [
        { type: t.meals.breakfast, meal: currentPlan.breakfast },
        { type: t.meals.lunch, meal: currentPlan.lunch },
        { type: t.meals.snack, meal: currentPlan.snack },
        { type: t.meals.dinner, meal: currentPlan.dinner }
      ];
      meals.forEach(m => {
        textSummary += `\n🍳 *${m.type}:* ${m.meal.name}`;
        if (m.meal.cookingTime) textSummary += ` (🔥 ${m.meal.cookingTime})`;
        textSummary += `\n_${m.meal.description}_`;
        textSummary += `\n\n*Ingredientes:* \n${m.meal.ingredients.join(', ')}`;
        if (m.meal.proTip) textSummary += `\n💡 *Tip:* ${m.meal.proTip}`;
        textSummary += `\n`;
      });
      textSummary += `\n📝 *${t.advice}:* ${currentPlan.advice}`;
    } else if (viewMode === 'weekly' && weeklyPlan) {
      textSummary += `📅 *${weeklyPlan.title || t.weekly}*\n`;
      weeklyPlan.days.forEach(day => {
        textSummary += `\n*${day.date.split(',')[0]}*\n`;
        textSummary += `🍴 ${day.breakfast.name}\n`;
        textSummary += `🍴 ${day.lunch.name}\n`;
        textSummary += `🍴 ${day.dinner.name}\n`;
      });
      textSummary += `\n📝 *${t.advice}:* ${weeklyPlan.weeklyAdvice}`;
    } else if (viewMode === 'monthly' && monthlyPlan) {
      textSummary += `📅 *${monthlyPlan.title || t.monthly}*\n`;
      textSummary += `\nHe generado tu plan de 30 días. Revisa el archivo adjunto para ver todas las recetas detalladas, lista de compras y consejos nutricionales.`;
    }

    // 2. Try Web Share API (Sharing the EXACT same HTML file)
    if (navigator.share) {
      try {
        const htmlContent = generateFullHTML(planData, viewMode);
        const fileName = `nutrimed-plan-${new Date().toISOString().split('T')[0]}.html`;
        const file = new File([htmlContent], fileName, { type: 'text/html' });

        if (navigator.canShare && navigator.canShare({ files: [file] })) {
          await navigator.share({
            files: [file],
            title: `NutriMed AI Plan - ${new Date().toLocaleDateString()}`,
            text: textSummary
          });
          return;
        }
  ...
  return (
    <div className="min-h-screen pb-20">
      ...
      {isLoading && (
        <div className="fixed inset-0 bg-stone-900/40 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
          <motion.div 
            ...
            className="bg-white p-8 rounded-3xl shadow-2xl max-w-sm w-full text-center space-y-6 border border-stone-100"
          >
            <div className="relative w-20 h-20 mx-auto">
              <div className="absolute inset-0 border-4 border-med-olive/20 rounded-full"></div>
              <div className="absolute inset-0 border-4 border-med-olive border-t-transparent rounded-full animate-spin"></div>
              <div className="absolute inset-0 flex items-center justify-center">
                <Utensils className="text-med-olive animate-pulse" size={32} />
              </div>
            </div>
            <div className="space-y-2">
              <h3 className="text-xl font-serif text-stone-800">
                {loadingType === 'daily' && t.loadingDaily}
                {loadingType === 'weekly' && t.loadingWeekly}
                {loadingType === 'monthly' && t.loadingMonthly}
                {loadingType === 'image' && t.generatingPhoto}
                {!loadingType && t.generating}
              </h3>
              <p className="text-stone-500 text-sm italic">
                {loadingType === 'monthly' ? (lang === 'es' ? "NutriMed AI está diseñando 28 días de nutrición personalizada..." : "NutriMed AI is designing 28 days of personalized nutrition...") : (lang === 'es' ? "Optimizando nutrientes para tu salud..." : "Optimizing nutrients for your health...")}
              </p>
            </div>
          </motion.div>
        </div>
      ...
      <main className="max-w-5xl mx-auto px-6 mt-12 space-y-12">
            ...
            {isEditingProfile ? (
              ...
              >
                <form onSubmit={handleSaveProfile} className="grid grid-cols-1 md:grid-cols-4 gap-6">
                  ...
                  <div className="md:col-span-2 space-y-2">
                    <label className="text-xs font-bold uppercase text-stone-700 tracking-wider">{t.favoriteCuisines} (e.g., Italian, Thai)</label>
                    <input name="favoriteCuisines" defaultValue={profile?.favoriteCuisines?.join(', ') || ''} className="input-med" placeholder="Italian, Japanese, etc." />
                  </div>
                  <div className="md:col-span-2 space-y-2">
                    <label className="text-xs font-bold uppercase text-stone-700 tracking-wider">{t.spiceLevel}</label>
                    <select name="spiceLevel" defaultValue={profile?.spiceLevel || 'medium'} className="input-med">
                      {Object.entries(t.spiceLevels).map(([key, label]) => (
                        <option key={key} value={key}>{label}</option>
                      ))}
                    </select>
                  </div>
          ...
          <section className="flex flex-col items-center gap-6">
            <div className="flex flex-wrap justify-center gap-3 md:gap-4">
              ...
              <button 
                ...
                className="btn-med bg-emerald-500 text-white hover:bg-emerald-600 border border-emerald-600 text-sm md:text-lg px-4 md:px-8 py-2 md:py-4 shadow-xl shadow-emerald-500/20"
              >
            ...
            <div className="flex flex-col items-center gap-4 mt-4">
              <h3 className="text-xs font-bold uppercase text-stone-400 tracking-widest">{t.generateSingle}</h3>
              <div className="flex flex-wrap justify-center gap-2">
                {(['breakfast', 'lunch', 'snack', 'dinner'] as const).map((type) => (
                  <button
                    ...
                    disabled={isLoading}
                    className="btn-med bg-white border border-stone-200 hover:border-med-olive text-stone-600 hover:text-med-olive text-xs px-4 py-2 rounded-full transition-all shadow-sm"
                  >
                    {t.meals[type]}
                  </button>
                ))}
            ...
            {successMessage && (
              <motion.div 
                ...
                animate={{ opacity: 1, y: 0 }}
                className="bg-emerald-50 text-emerald-700 px-4 py-2 rounded-lg border border-emerald-200 text-sm font-medium flex items-center gap-2"
              >
        ...
        {/* Single Meal Display */}
        <AnimatePresence mode="wait">
          {singleMeal && viewMode === 'daily' && (
            <motion.section
              key="single-meal-view"
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: -20 }}
              className="space-y-8"
            >
              <div className="flex items-center justify-between border-b border-stone-200 pb-4">
                <h2 className="text-4xl font-serif flex items-center gap-3">
                  <ChefHat className="text-med-olive" size={32} />
                  {t.generateSingle}
                </h2>
                <button 
                  onClick={() => setSingleMeal(null)}
              ...
              <div className="max-w-2xl mx-auto">
                <MealCard 
                  type={t.meals[singleMeal.type as keyof typeof t.meals]} 
                  meal={singleMeal.meal} 
                  t={t} 
                  onExclude={(ing) => handleExcludeIngredient(ing)} 
                  onRegenerate={() => handleGenerateSingleMeal(singleMeal.type as any)} 
                  onLike={() => handleLikeMeal(singleMeal.meal.name)} 
                  onDislike={() => handleDislikeMeal(singleMeal.meal.name)} 
                  isLoading={isLoading} 
                />
              </div>
            </motion.section>
        ...
        <AnimatePresence mode="wait">
          {viewMode === 'saved' && (
            ...
            >
              <div className="flex items-center justify-between border-b border-stone-200 pb-4">
                <h2 className="text-4xl font-serif flex items-center gap-3">
                  <Bookmark className="text-med-olive" size={32} />
                  {t.savedPlans}
                </h2>
                <button 
              ...
              <div className="space-y-12">
                      ...
                      {savedMonthlyPlans.map((plan, idx) => (
                        <div key={idx} className="card-med p-6 flex justify-between items-center group hover:border-med-olive transition-all">
                          ...
                          >
                            ...
                            <p className="text-xs text-stone-400 font-bold uppercase tracking-widest">{plan.weeks.length} {t.week}s</p>
                          </div>
                          <button 
                            onClick={() => handleDeleteSavedPlan(idx, 'monthly')}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                      {savedWeeklyPlans.map((plan, idx) => (
                        <div key={idx} className="card-med p-6 flex justify-between items-center group hover:border-med-olive transition-all">
                          ...
                          >
                            <h4 className="text-xl font-serif">{plan.title}</h4>
                            <p className="text-xs text-stone-400 font-bold uppercase tracking-widest">{plan.days[0].date.split(',')[0]}</p>
                          </div>
                          <button 
                            onClick={() => handleDeleteSavedPlan(idx, 'weekly')}
                            className="text-stone-300 hover:text-red-500 transition-colors p-2"
                          >
                            <Trash2 size={20} />
                          </button>
                ...
                {savedWeeklyPlans.length === 0 && savedMonthlyPlans.length === 0 && (
                  <div className="py-20 text-center space-y-4">
                    <Bookmark size={48} className="mx-auto text-stone-200" />
                    <p className="text-stone-500 italic">{t.noSavedPlans}</p>
                  </div>
                )}
              </div>
            </motion.section>
          ...
          {currentPlan && viewMode !== 'history' && (
            ...
            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <MealCard type={t.meals.breakfast} meal={currentPlan.breakfast} t={t} onExclude={(ing) => handleExcludeIngredient(ing, 'breakfast')} onRegenerate={() => handleRegenerateMeal('breakfast')} onLike={() => handleLikeMeal(currentPlan.breakfast.name)} onDislike={() => handleDislikeMeal(currentPlan.breakfast.name)} isLoading={isLoading} />
                <MealCard type={t.meals.lunch} meal={currentPlan.lunch} t={t} onExclude={(ing) => handleExcludeIngredient(ing, 'lunch')} onRegenerate={() => handleRegenerateMeal('lunch')} onLike={() => handleLikeMeal(currentPlan.lunch.name)} onDislike={() => handleDislikeMeal(currentPlan.lunch.name)} isLoading={isLoading} />
                <MealCard type={t.meals.snack} meal={currentPlan.snack} t={t} onExclude={(ing) => handleExcludeIngredient(ing, 'snack')} onRegenerate={() => handleRegenerateMeal('snack')} onLike={() => handleLikeMeal(currentPlan.snack.name)} onDislike={() => handleDislikeMeal(currentPlan.snack.name)} isLoading={isLoading} />
                <MealCard type={t.meals.dinner} meal={currentPlan.dinner} t={t} onExclude={(ing) => handleExcludeIngredient(ing, 'dinner')} onRegenerate={() => handleRegenerateMeal('dinner')} onLike={() => handleLikeMeal(currentPlan.dinner.name)} onDislike={() => handleDislikeMeal(currentPlan.dinner.name)} isLoading={isLoading} />
              </div>
            ...
            >
              <div className="flex items-center justify-between border-b border-stone-200 pb-4">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <button 
                    onClick={() => handleSaveToArchive(weeklyPlan)}
                    className="btn-med border border-emerald-200 bg-emerald-50 text-emerald-700 hover:bg-emerald-100 px-3 py-1.5 md:px-4 md:py-2 text-xs md:text-sm"
                  >
                    <Save size={16} />
                    <span className="uppercase font-bold tracking-tighter">{t.savePlan}</span>
                  </button>
                </div>
              </div>
              ...
              <div className="space-y-12">
                {weeklyPlan.days.map((day, i) => (
                  <div key={i} className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <MealCard type={t.meals.breakfast} meal={day.breakfast} t={t} onExclude={(ing) => handleExcludeIngredient(ing, 'breakfast', i)} onRegenerate={() => handleRegenerateWeeklyMeal(i, 'breakfast')} onLike={() => handleLikeMeal(day.breakfast.name)} onDislike={() => handleDislikeMeal(day.breakfast.name)} isLoading={isLoading} />
                      <MealCard type={t.meals.lunch} meal={day.lunch} t={t} onExclude={(ing) => handleExcludeIngredient(ing, 'lunch', i)} onRegenerate={() => handleRegenerateWeeklyMeal(i, 'lunch')} onLike={() => handleLikeMeal(day.lunch.name)} onDislike={() => handleDislikeMeal(day.lunch.name)} isLoading={isLoading} />
                      <MealCard type={t.meals.snack} meal={day.snack} t={t} onExclude={(ing) => handleExcludeIngredient(ing, 'snack', i)} onRegenerate={() => handleRegenerateWeeklyMeal(i, 'snack')} onLike={() => handleLikeMeal(day.snack.name)} onDislike={() => handleDislikeMeal(day.snack.name)} isLoading={isLoading} />
                      <MealCard type={t.meals.dinner} meal={day.dinner} t={t} onExclude={(ing) => handleExcludeIngredient(ing, 'dinner', i)} onRegenerate={() => handleRegenerateWeeklyMeal(i, 'dinner')} onLike={() => handleLikeMeal(day.dinner.name)} onDislike={() => handleDislikeMeal(day.dinner.name)} isLoading={isLoading} />
                    </div>
                  </div>
                ))}
              </div>
            ...
            >
              <div className="flex flex-wrap items-center justify-between border-b border-stone-200 pb-4 gap-4">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <button 
                    onClick={() => handleSaveToArchive(monthlyPlan)}
                    className="btn-med border border-emerald-200 bg-emerald-50 text-emerald-700 hover:bg-emerald-100 px-4 py-2"
                  >
                    <Save size={18} />
                    <span className="uppercase text-xs font-bold tracking-tighter">{t.savePlan}</span>
                  </button>
                </div>
              </div>
                ...
                <div key={weekIdx} className="space-y-8">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    {week.days.map((day, i) => (
                      <div 
                        key={i} 
                        className="card-med p-4 hover:shadow-md transition-shadow cursor-pointer group"
                        onClick={() => setSelectedDay({ weekIdx, dayIdx: i, plan: day })}
                      >
                        <div className="flex justify-between items-start border-b border-stone-100 pb-2 mb-3">
                          <h5 className="font-bold text-med-olive">{day.date.split(',')[0]}</h5>
                          <ArrowRight size={14} className="text-stone-300 group-hover:text-med-olive transition-colors" />
                        </div>
                        <div className="space-y-2 text-xs">
                          <p><strong>{t.meals.breakfast}:</strong> {day.breakfast.name}</p>
                          <p><strong>{t.meals.lunch}:</strong> {day.lunch.name}</p>
                          <p><strong>{t.meals.dinner}:</strong> {day.dinner.name}</p>
                        </div>
                      </div>
                    ))}
                  </div>
      ...
      <AnimatePresence>
        {selectedDay && (
          <motion.div 
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-[110] bg-stone-900/90 backdrop-blur-sm flex items-center justify-center p-4 md:p-8"
          >
            <motion.div 
              initial={{ scale: 0.9, y: 20 }}
              animate={{ scale: 1, y: 0 }}
              exit={{ scale: 0.9, y: 20 }}
              className="bg-stone-50 w-full max-w-4xl max-h-[90vh] rounded-2xl overflow-hidden flex flex-col shadow-2xl"
            >
              <div className="p-6 border-b border-stone-200 flex items-center justify-between bg-white">
                <h3 className="text-2xl font-serif text-med-olive">{selectedDay.plan.date}</h3>
                <button 
              ...
              <div className="flex-1 overflow-y-auto p-6 md:p-8 space-y-8">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <MealCard 
                    type={t.meals.breakfast} 
                    meal={selectedDay.plan.breakfast} 
                    t={t} 
                    onExclude={(ing) => handleExcludeIngredient(ing, 'breakfast', selectedDay.dayIdx, selectedDay.weekIdx)} 
                    onRegenerate={() => {
                      if (selectedDay.weekIdx !== undefined) {
                        handleRegenerateMonthlyMeal(selectedDay.weekIdx, selectedDay.dayIdx, 'breakfast');
                      } else {
                        handleRegenerateWeeklyMeal(selectedDay.dayIdx, 'breakfast');
                      }
                    }} 
                    onLike={() => handleLikeMeal(selectedDay.plan.breakfast.name)}
                    onDislike={() => handleDislikeMeal(selectedDay.plan.breakfast.name)}
                    isLoading={isLoading} 
                  />
                  <MealCard 
                    type={t.meals.lunch} 
                    meal={selectedDay.plan.lunch} 
                    t={t} 
                    onExclude={(ing) => handleExcludeIngredient(ing, 'lunch', selectedDay.dayIdx, selectedDay.weekIdx)} 
                    onRegenerate={() => {
                      if (selectedDay.weekIdx !== undefined) {
                        handleRegenerateMonthlyMeal(selectedDay.weekIdx, selectedDay.dayIdx, 'lunch');
                      } else {
                        handleRegenerateWeeklyMeal(selectedDay.dayIdx, 'lunch');
                      }
                    }} 
                    onLike={() => handleLikeMeal(selectedDay.plan.lunch.name)}
                    onDislike={() => handleDislikeMeal(selectedDay.plan.lunch.name)}
                    isLoading={isLoading} 
                  />
                  <MealCard 
                    type={t.meals.snack} 
                    meal={selectedDay.plan.snack} 
                    t={t} 
                    onExclude={(ing) => handleExcludeIngredient(ing, 'snack', selectedDay.dayIdx, selectedDay.weekIdx)} 
                    onRegenerate={() => {
                      if (selectedDay.weekIdx !== undefined) {
                        handleRegenerateMonthlyMeal(selectedDay.weekIdx, selectedDay.dayIdx, 'snack');
                      } else {
                        handleRegenerateWeeklyMeal(selectedDay.dayIdx, 'snack');
                      }
                    }} 
                    onLike={() => handleLikeMeal(selectedDay.plan.snack.name)}
                    onDislike={() => handleDislikeMeal(selectedDay.plan.snack.name)}
                    isLoading={isLoading} 
                  />
                  <MealCard 
                    type={t.meals.dinner} 
                    meal={selectedDay.plan.dinner} 
                    t={t} 
                    onExclude={(ing) => handleExcludeIngredient(ing, 'dinner', selectedDay.dayIdx, selectedDay.weekIdx)} 
                    onRegenerate={() => {
                      if (selectedDay.weekIdx !== undefined) {
                        handleRegenerateMonthlyMeal(selectedDay.weekIdx, selectedDay.dayIdx, 'dinner');
                      } else {
                        handleRegenerateWeeklyMeal(selectedDay.dayIdx, 'dinner');
                      }
                    }} 
                    onLike={() => handleLikeMeal(selectedDay.plan.dinner.name)}
                    onDislike={() => handleDislikeMeal(selectedDay.plan.dinner.name)}
                    isLoading={isLoading} 
                  />
                </div>
              </div>
      ...
      <div className="grid grid-cols-2 gap-8 mb-8">
        <div className="space-y-4">
          ...
          <div className="flex justify-between items-start">
            ...
            <div className="flex gap-2 text-[8px] font-bold text-stone-400">
              {plan.breakfast.prepTime && <span>⏱ {plan.breakfast.prepTime}</span>}
              {plan.breakfast.cookingTime && <span>🔥 {plan.breakfast.cookingTime}</span>}
            </div>
          </div>
          <p className="text-xs text-stone-600">{plan.breakfast.description}</p>
          {plan.breakfast.proTip && (
            <div className="bg-med-cream/20 p-2 rounded border-l-2 border-med-olive text-[9px] italic text-stone-600">
              <span className="font-bold not-italic uppercase text-[7px] text-med-olive block mb-0.5">{t.proTip}</span>
              {plan.breakfast.proTip}
            </div>
          )}
        ...
        <div className="space-y-4">
          ...
          <div className="flex justify-between items-start">
            ...
            <div className="flex gap-2 text-[8px] font-bold text-stone-400">
              {plan.lunch.prepTime && <span>⏱ {plan.lunch.prepTime}</span>}
              {plan.lunch.cookingTime && <span>🔥 {plan.lunch.cookingTime}</span>}
            </div>
          </div>
        ...
        <div className="space-y-4">
          ...
          <div className="flex justify-between items-start">
            ...
            <div className="flex gap-2 text-[8px] font-bold text-stone-400">
              {plan.snack.prepTime && <span>⏱ {plan.snack.prepTime}</span>}
              {plan.snack.cookingTime && <span>🔥 {plan.snack.cookingTime}</span>}
            </div>
          </div>
        ...
        <div className="space-y-4">
          ...
          <div className="flex justify-between items-start">
            ...
            <div className="flex gap-2 text-[8px] font-bold text-stone-400">
              {plan.dinner.prepTime && <span>⏱ {plan.dinner.prepTime}</span>}
              {plan.dinner.cookingTime && <span>🔥 {plan.dinner.cookingTime}</span>}
            </div>
          </div>
              ...
              <table className="w-full border-collapse text-[10px]">
                ...
                <tbody>
                  <tr>
                    ...
                    <div className="flex justify-between items-center text-[7px] text-stone-400 mt-0.5 italic">
                      {day.breakfast.prepTime && <span>⏱ {day.breakfast.prepTime}</span>}
                      {day.breakfast.cookingTime && <span>🔥 {day.breakfast.cookingTime}</span>}
                    </div>
                      </td>
                    ))}
                  </tr>
                  <tr>
                    ...
                    <div className="flex justify-between items-center text-[7px] text-stone-400 mt-0.5 italic">
                      ...
                      {day.lunch.cookingTime && <span>🔥 {day.lunch.cookingTime}</span>}
                    </div>
                      </td>
                    ))}
                  </tr>
                  <tr>
                    ...
                    <div className="flex justify-between items-center text-[7px] text-stone-400 mt-0.5 italic">
                      ...
                      {day.dinner.cookingTime && <span>🔥 {day.dinner.cookingTime}</span>}
                    </div>
                      </td>
                    ))}
                  </tr>
                ...
                <td key={i} className="border border-stone-200 p-3 align-top">
                  ...
                  <div className="flex justify-between items-center text-[9px] text-stone-500">
                    ...
                    <div className="flex gap-1">
                      ...
                      {day.snack.cookingTime && <span>🔥 {day.snack.cookingTime}</span>}
                    </div>
                  </div>
                </td>
              ))}
...
function MealCard({ type, meal, t, onExclude, onRegenerate, onLike, onDislike, isLoading }: { type: string, meal: Meal, t: any, onExclude?: (ing: string) => void, onRegenerate?: () => void, onLike?: () => void, onDislike?: () => void, isLoading?: boolean }) {
    ...
    <div className="card-med flex flex-col group/card relative">
      ...
      <div className="p-6 border-b border-stone-100 bg-stone-50/50 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3 text-[10px] font-bold text-stone-400">
          {meal.prepTime && (
            <div className="flex items-center gap-1 text-med-olive bg-med-olive/5 px-2 py-0.5 rounded" title={t.prepTime}>
              <Clock size={10} />
              <span>{meal.prepTime}</span>
            </div>
          )}
          {meal.cookingTime && (
            <div className="flex items-center gap-1 text-orange-600 bg-orange-50 px-2 py-0.5 rounded" title={t.cookingTime}>
              <Flame size={10} />
      ...
      <div className="p-6 flex-1">
        ...
        <div className="flex flex-wrap gap-2 mb-6">
          {meal.ingredients.map((ing: string, i: number) => (
            <div key={i} className="flex items-center gap-1 px-2 py-1 bg-med-cream text-med-olive text-[10px] rounded-md font-medium group/ing border border-med-olive/10">
              {ing}
              {onExclude && (
                <button 
        ...
        <div className="flex gap-2 mb-4">
          ...
          <div className="flex gap-1">
            {onLike && (
              <button 
                onClick={onLike}
                className="btn-med border border-stone-200 text-stone-400 hover:text-emerald-600 hover:bg-emerald-50 px-3 py-2"
                title={t.like}
              >
                <ThumbsUp size={16} />
              </button>
            ...
            {onDislike && (
              <button 
                onClick={onDislike}
                className="btn-med border border-stone-200 text-stone-400 hover:text-red-600 hover:bg-red-50 px-3 py-2"
                title={t.dislike}
              >
                <ThumbsDown size={16} />
              </button>
          ...
          {isOpen && (
            ...
            >
              <div className="pt-6 space-y-6">
                {meal.proTip && (
                  <div className="bg-med-cream/30 border-l-4 border-med-olive p-4 rounded-r-xl italic text-sm text-stone-700 flex gap-3">
                    <Lightbulb className="text-med-olive shrink-0" size={20} />
                    <div>
                      <span className="font-bold block not-italic text-xs uppercase tracking-widest text-med-olive mb-1">{t.proTip}</span>
                      {meal.proTip}
                    </div>
                  </div>
                )}
